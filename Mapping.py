...
def hisat2_builder(genome_file_path, base_string):
    ...
    if not os.path.isfile(base_string + '.1.ht2'):
        print cmd_string
        print "Hisat2-builder is running, hur hur hur."
        cmd_string = 'hisat2-build %s %s' % (genome_file_path, base_string)
...
def create_sam_base_string(read_file_path):
    ...
    sam_base_string = sam_base_string.split('_')
    sam_base_string = sam_base_string[0] + '.sam'
...
def hisat2_aligner(base_string, read1_path, read2_path, sam_base_string):
    ...
    if not os.path.isfile(sam_base_string):
        cmd_string = 'hisat2 -t --no-unal --dta-cufflinks -x %s -1 %s -2 %s -S %s' % (base_string,\
                    read1_path, read2_path, sam_base_string)
        print cmd_string
        print "Hisat2-aligner is running, hur hur hur."
        execute_on_command_line(cmd_string)
